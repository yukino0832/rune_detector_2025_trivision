 ...
 */
Lightline::Lightline(const std::vector<cv::Point>& contour, const cv::Rect2f& localRoi, const cv::Rect2f& globalRoi):
    m_contour(contour),
 ...
 * @param[in] roi           roi 用来设置正确的中心及角点
 */
Circlelight::Circlelight(const cv::Vec3f& circle, const cv::Rect2f& localRoi, const cv::Rect2f& globalRoi) {
...
void findArrowLightlines(const cv::Mat& binary, std::vector<Lightline>& lightlines, const cv::Rect2f& roi) {
    ...
    for (const auto& contour: contours)
    {
        ...
        // 判断面积
        if (inRange(lightline.m_area, Param::MIN_ARROW_LIGHTLINE_AREA, Param::MAX_ARROW_LIGHTLINE_AREA) == false)
        {
            continue;
        }
        // 判断长宽比
...
bool findArrow(Arrow& arrow, const std::vector<Lightline>& lightlines, const cv::Rect2f& roi) {
    ...
    for (auto label: labels)
    {
        ...
        auto iter = std::find_if(data.begin(), data.end(), [label](const std::pair<int, int>& unit) { return unit.first == label; });
        if (iter == data.end())
        {
 ...
 * @return false
 */
bool findCenterLightlines(const cv::Mat& image, std::vector<Lightline>& lightlines, const cv::Rect2f& globalRoi, const cv::Rect2f& localRoi) {
    // 寻找轮廓
    std::vector<std::vector<cv::Point>> contours;
 ...
 * @return false
 */
bool findCenterR(CenterR& center, const std::vector<Lightline>& lightlines, const Arrow& arrow, const Armor& armor) {
    // 设置中心 R 到装甲板中心的距离范围
    ...
    Lightline target {
        *std::max_element(filteredLightlines.begin(), filteredLightlines.end(), [](const Lightline& l1, const Lightline& l2) { return l1.m_area < l2.m_area; })
    };
    center.set(target);
    return true;
}
...
void resetRoi(cv::Rect2f& rect, const cv::Mat& mat) {
    resetRoi(rect, mat.rows, mat.cols);
}
...
void resetRoi(cv::Rect2f& rect, const cv::Rect2f& lastRoi) {
    resetRoi(rect, lastRoi.height, lastRoi.width);
}
...
double calAngleBetweenLightlines(const Lightline& l1, const Lightline& l2) {
    ...
    for (std::size_t i = 0; i < 2; ++i)
    {
        for (std::size_t j = 0; j < 4; ++i)
        {
            ...
            {
                if (std::abs(pointPointDistance(pointsArray.at(i).at(j), pointsArray.at(i).at(k)) - lengths.at(i)) < eps)
                {
                    flag = true;
 ...
 */
Detector::Detector(): // 全局 roi 初始设置为图像的全部范围，注意图像大小要与下面两个参数对应
...
void Detector::preprocess(const Frame& frame) {
    ...
    // 对灰度图进行二值化
    cv::threshold(temp, m_imageArrow, Param::ARROW_BRIGHTNESS_THRESHOLD, Param::MAX_BRIGHTNESS, cv::THRESH_BINARY);
...
/**
 * @brief 计算圆形灯条的外接矩形的四个角点
 * 
 */
void Detector::setArmor() {
    double distance_armor_centerR { pointPointDistance(m_armor.m_center, m_centerR.m_center) };
    double dx { (m_centerR.m_x - m_armor.m_x) / distance_armor_centerR };
    double dy { (m_centerR.m_y - m_armor.m_y) / distance_armor_centerR };
    std::vector<cv::Point2f> points;
    points.emplace_back(m_armor.m_x + m_armor.m_circlelight.m_radius * dx - m_armor.m_circlelight.m_radius * dy, m_armor.m_y + m_armor.m_circlelight.m_radius * dy + m_armor.m_circlelight.m_radius * dx);
    points.emplace_back(m_armor.m_x + m_armor.m_circlelight.m_radius * dx + m_armor.m_circlelight.m_radius * dy, m_armor.m_y + m_armor.m_circlelight.m_radius * dy - m_armor.m_circlelight.m_radius * dx);
    points.emplace_back(m_armor.m_x - m_armor.m_circlelight.m_radius * dx - m_armor.m_circlelight.m_radius * dy, m_armor.m_y - m_armor.m_circlelight.m_radius * dy + m_armor.m_circlelight.m_radius * dx);
    points.emplace_back(m_armor.m_x - m_armor.m_circlelight.m_radius * dx + m_armor.m_circlelight.m_radius * dy, m_armor.m_y - m_armor.m_circlelight.m_radius * dy - m_armor.m_circlelight.m_radius * dx);
    std::sort(points.begin(), points.end(), [](const cv::Point2f& a, const cv::Point2f& b) {
        return a.x < b.x; // 按 x 坐标升序排序
    });
    if (m_armor.m_x < m_centerR.m_x)
    {
        if (m_armor.m_y < m_centerR.m_y)
        {
            m_armor.m_tl = points.at(0);
            m_armor.m_tr = points.at(1);
            m_armor.m_bl = points.at(2);
            m_armor.m_br = points.at(3);
            if (m_armor.m_bl.y < m_armor.m_tr.y)
            {
                std::swap(m_armor.m_tr, m_armor.m_bl);
            }
        } else
        {
            m_armor.m_tr = points.at(0);
            m_armor.m_tl = points.at(1);
            m_armor.m_br = points.at(2);
            m_armor.m_bl = points.at(3);
            if (m_armor.m_tl.y < m_armor.m_br.y)
            {
                std::swap(m_armor.m_tl, m_armor.m_br);
            }
        }
    } else
    {
        if (m_armor.m_y < m_centerR.m_y)
        {
            m_armor.m_bl = points.at(0);
            m_armor.m_br = points.at(1);
            m_armor.m_tl = points.at(2);
            m_armor.m_tr = points.at(3);
            if (m_armor.m_br.y < m_armor.m_tl.y)
            {
                std::swap(m_armor.m_tl, m_armor.m_br);
            }
        } else
        {
            m_armor.m_br = points.at(0);
            m_armor.m_bl = points.at(1);
            m_armor.m_tr = points.at(2);
            m_armor.m_tl = points.at(3);
            if (m_armor.m_tr.y < m_armor.m_bl.y)
            {
                std::swap(m_armor.m_tr, m_armor.m_bl);
            }
        }
    }
    cv::circle(m_imageShow, m_armor.m_bl, 1, Param::WHITE, 2);
    cv::circle(m_imageShow, m_armor.m_br, 1, Param::WHITE, 2);
    cv::circle(m_imageShow, m_armor.m_tl, 1, Param::WHITE, 2);
    cv::circle(m_imageShow, m_armor.m_tr, 1, Param::WHITE, 2);
    cv::putText(m_imageShow, "1", cv::Point(m_armor.m_tr.x, m_armor.m_tr.y), cv::FONT_HERSHEY_SIMPLEX, 1, Param::YELLOW, 2, cv::LINE_AA);
    cv::putText(m_imageShow, "2", cv::Point(m_armor.m_tl.x, m_armor.m_tl.y), cv::FONT_HERSHEY_SIMPLEX, 1, Param::YELLOW, 2, cv::LINE_AA);
    cv::putText(m_imageShow, "3", cv::Point(m_armor.m_bl.x, m_armor.m_bl.y), cv::FONT_HERSHEY_SIMPLEX, 1, Param::YELLOW, 2, cv::LINE_AA);
    cv::putText(m_imageShow, "4", cv::Point(m_armor.m_br.x, m_armor.m_br.y), cv::FONT_HERSHEY_SIMPLEX, 1, Param::YELLOW, 2, cv::LINE_AA);

    MUTEX.lock();
    std::cout << "R标坐标:" << m_centerR.m_center << std::endl;
...
void Detector::setGlobalRoi() {
    double width { Param::GLOBAL_ROI_LENGTH_RATIO * 2 * pointPointDistance(m_armor.m_center, m_centerR.m_center) };
 ...
 * @param[in] localRoi      局部 roi
 */
void Detector::draw(const Lightline& lightline, const cv::Scalar& color, const int thickness, const cv::Rect2f& localRoi) {
 ...
 * @param[in] localRoi      局部 roi
 */
void Detector::draw(const cv::RotatedRect& rotatedRect, const cv::Scalar& color, const int thickness, const cv::Rect2f& localRoi) {
 ...
 * @param[in] localRoi      局部 roi
 */
void Detector::draw(const cv::Rect2f& rect, const cv::Scalar& color, const int thickness, const cv::Rect2f& localRoi) {
    cv::Rect2f temp = rect;
 ...
 * @param[in] localRoi      局部 roi
 */
void Detector::draw(const std::vector<cv::Point2f>& points, const cv::Scalar& color, const int thickness, const cv::Rect2f& localRoi) {
    for (std::size_t i = 0; i < points.size(); ++i)
    {
        cv::line(m_imageShow, points[i] + localRoi.tl() + m_globalRoi.tl(), points[(i + 1) % points.size()] + localRoi.tl() + m_globalRoi.tl(), color, thickness);
    }
}
 ...
 * @param[in] localRoi      局部 roi
 */
void Detector::draw(const cv::Point2f* points, const std::size_t size, const cv::Scalar& color, const int thickness, const cv::Rect2f& localRoi) {
    for (std::size_t i = 0; i < size; ++i)
    {
        cv::line(m_imageShow, points[i] + localRoi.tl() + m_globalRoi.tl(), points[(i + 1) % size] + localRoi.tl() + m_globalRoi.tl(), color, thickness);
    }
}
